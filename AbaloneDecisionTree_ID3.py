        ...
        for Campo in range(NumCampos):
            ...
            for i in range (TopeMemoria-2):
                ...
                for w in range (NumClases):
                    ...
                    TotClases= TotClases + TabVotos[Campo,i,w]
                for j in range (NumClases):
                    if ((TabVotos[Campo,i,j] != 0.0) and (TotClases != 0.0) ):
                        P=TabVotos[Campo,i,j]/TotClases
                        
                        EntroCampo= EntroCampo + (TotClases/float(End - Start )) * P*math.log(1.0/P,2)
                        # TotalClases=TotalClases+TotClases
            print("entropy field "+  str(Campo)+ " = " + str(EntroCampo) )
...
def AbaloneNaiveBayes_train_test (TabVotos, ContClase,Max,Min, Start, End,SwTestTraining, Y_train, X_Train, Y_test, X_test, W): 
                ...
                for linea1 in f:
                     ...
                     for y in range(NumCampos):
                         ...
                         if ( (indice > (TopeMemoria-2)) or  (indice < 0)):	
                                   ...
                                   indice=TopeMemoria
                         swHayClasesSeguras=0
                         ClaseSegura=-1
                         
                         for x in range (NumClases):
                        # print("Recupera indice = "+str(i) + "Clase= " + str(x) + "Valor = " +str(TabVotos[Campo,i,w]))
                            if TabVotos[z,indice,x] > 0.0  :
                                 swHayClasesSeguras= swHayClasesSeguras+1
                                 ClaseSegura=x
                                 
                         if  swHayClasesSeguras==1 :
                             if (ClaseSegura==Clase):
                                   TotalHitsSegura= TotalHitsSegura + 1.0
                                   # print("ACIERTA SEGURA Registro = " + str(Conta) + "Clase Segura " + str(ClaseSegura) + " Clase real =" + str(Clase))  
                                   if (SwInicial==0):
                                         SwInicial=1
                                         Y=[float(Clase)]
                                         X=[float(Clase)]
                                         ...
                                         X.append(float(Clase))
                                   strlinea1=strlinea1+",0"+"\n"
                                   w.write(strlinea1)
                                   break
                             else:
                                   TotalFailuresSegura=TotalFailuresSegura+1.0
                                   # print("FALLO SEGURA Registro = " + str(Conta) + "Clase Segura " + str(ClaseSegura) + " Clase real =" + str(Clase)) 
                                   continue
                     if ( swHayClasesSeguras==1) and (ClaseSegura==Clase): continue 
                     for x in range(NumClases):
        ...
        f.close 
        w.close 
        print ("TOTAL SURE HITS=" + str(TotalHitsSegura) +   " TOTAL SURE  FAILURES=" + str(TotalFailuresSegura))
